          ...
          <table className="w-full text-sm">
            ...
            <tbody>
              {rows.map((w) => {
                ...
                const offerTxt = w.offer_expires_at ? new Date(w.offer_expires_at).toLocaleString() : "—";
                ...
                return (
                  <tr key={w.id} className="border-b last:border-b-0">
                    ...
                    <td className="px-4 py-3 text-slate-700">{new Date(w.start_time).toLocaleString()}</td>
                    <td className="px-4 py-3 text-slate-700">{new Date(w.end_time).toLocaleString()}</td>
                    <td className="px-4 py-3">
                      <span className="rounded-full bg-slate-100 px-2 py-1 text-xs text-slate-700 ring-1 ring-slate-200">
                        {w.status}
                      </span>
                    </td>
                    <td className="px-4 py-3 text-slate-700">{offerTxt}</td>
                    <td className="px-4 py-3">
