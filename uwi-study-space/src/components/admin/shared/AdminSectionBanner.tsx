...
export default function AdminSectionBanner({
  ...
  const roleLabel = mode === "super_admin" ? "Super Admin" : "Admin";
      ...
      <div className="px-4 sm:px-6 lg:px-8">
        ...
        <div className="py-6 flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
          <div>
            ...
            <p className="mt-1.5 text-sm text-[#6B7280] max-w-lg">{description}</p>
          </div>
          ...
          >
            <span className="hover:text-[#003595] cursor-default transition-colors">{roleLabel}</span>
            ...
            <span className="font-semibold text-[#003595]">{breadcrumbLabel}</span>
          </nav>
        </div>
      </div>
    </div>
