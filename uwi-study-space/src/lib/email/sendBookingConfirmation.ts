...
export async function sendBookingConfirmation(input: BookingConfirmationInput) {
  ...
  if (!to) {
    console.warn("[email.confirmation] No recipient resolved");
    return null;
  }

  const html = renderEmailShell({
    title: "Booking Confirmed",
    preview: `Your booking for ${input.roomName} has been confirmed.`,
    bodyHtml: `
      ...
      <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="margin:0 0 16px;border-collapse:collapse;">
        ...
        <tr><td style="padding:8px 0;font-weight:700;">Purpose:</td><td style="padding:8px 0;">${escapeHtml(input.purpose || "-")}</td></tr>
      </table>
      <p style="margin:0 0 16px;font-size:15px;line-height:1.7;">
        Booking ID: <strong>${String(input.bookingId)}</strong>
      </p>
      <p style="margin:0;font-size:15px;line-height:1.7;">
        View your bookings in the app:
  ...
  return resend.emails.send({
    from,
    to,
    subject: `Booking confirmed - ${input.roomName}`,
    html,
  });
}
