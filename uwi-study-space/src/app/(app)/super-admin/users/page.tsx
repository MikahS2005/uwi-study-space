...
function RoleBadge({ role }: { role: Role }) {
  ...
  return (
    <span className={`inline-flex items-center rounded-full px-2.5 py-0.5 text-[10px] font-bold tracking-wide uppercase ring-1 ${styles[role]}`}>
      {labels[role]}
    </span>
  );
}
...
function SkeletonRow() {
  return (
    <tr className="animate-pulse border-b border-[#F3F4F6]">
      ...
      <td className="py-3.5 px-4 hidden md:table-cell"><div className="h-5 w-20 rounded-full bg-[#F3F4F6]" /></td>
      <td className="py-3.5 px-4 hidden lg:table-cell"><div className="h-4 w-28 rounded bg-[#F3F4F6]" /></td>
      <td className="py-3.5 pl-4 pr-5"><div className="h-8 w-32 rounded-lg bg-[#F3F4F6] ml-auto" /></td>
    </tr>
  );
}
...
───────────────────────────────────────────────────────────── */
function ScopeModal({
  open, onClose, user, departments, onSave,
}: {
  open: boolean;
  onClose: () => void;
  user: UserRow | null;
      ...
      <div className="w-full max-w-md bg-white rounded-xl shadow-2xl border border-[#E5E7EB] overflow-hidden">
        ...
        <div className="px-5 py-4 border-b border-[#E5E7EB] bg-[#F9FAFB]">
          ...
          <p className="mt-0.5 text-xs text-[#6B7280]">
            {user.fullName || user.email} — select departments this admin can manage.
          </p>
        </div>
        ...
        <div className="px-5 py-4 max-h-72 overflow-y-auto space-y-1.5">
          {departments.length === 0 ? (
            <p className="text-xs text-[#9CA3AF] text-center py-4">No departments found.</p>
          ) : (
            departments.map((d) => {
        ...
        <div className="flex gap-2.5 px-5 pb-5 border-t border-[#E5E7EB] pt-4">
          ...
          <button
            disabled={saving}
            onClick={async () => { setSaving(true); try { await onSave(selected); onClose(); } finally { setSaving(false); } }}
          ...
          >
            {saving ? <><Spinner light size={14} /> Saving…</> : "Save Permissions"}
          </button>
        </div>
      </div>
    </div>
...
export default function SuperAdminUsersPage() {
  ...
  const filtered = useMemo(() => {
    ...
    return users.filter((u) => {
      ...
      return [u.fullName, u.email, u.uwiId, u.departmentName ?? ""]
  ...
  async function updateRole(targetUserId: string, newRole: Role) {
    ...
    try {
      ...
      if (!r.ok) { const j = await r.json().catch(() => ({})); alert(j.error ?? "Failed to update role"); return; }
      await refresh();
    ...
    <div className="min-h-screen bg-[#F9FAFB]">
      ...
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-8 space-y-6">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
          {(["student", "staff", "admin", "super_admin"] as Role[]).map((role) => (
            <button
              key={role}
            ...
            >
              ...
              <p style={{ fontFamily: "Georgia, serif" }}
                className={`text-2xl font-bold mt-0.5 ${roleFilter === role ? "text-[#003595]" : "text-[#1F2937]"}`}>
                {counts[role]}
              </p>
            </button>
          ))}
        ...
        <div className="bg-white rounded-xl border border-[#E5E7EB] shadow-sm overflow-hidden">
          <div className="px-5 py-3.5 border-b border-[#E5E7EB] bg-[#F9FAFB]">
            <h2 className="text-xs font-bold tracking-[0.12em] uppercase text-[#374151]">Filter & Search</h2>
          </div>
        ...
        <div className="bg-white rounded-xl border border-[#E5E7EB] shadow-sm overflow-hidden">
          <div className="px-5 py-3.5 border-b border-[#E5E7EB] bg-[#F9FAFB] flex items-center justify-between">
            <h2 className="text-xs font-bold tracking-[0.12em] uppercase text-[#374151]">All Users</h2>
            {!loading && (
              <span className="text-xs text-[#9CA3AF]">{filtered.length} of {users.length}</span>
            )}
          </div>
            ...
            <table className="w-full text-sm" aria-label="Users table">
              <thead>
                <tr className="border-b border-[#E5E7EB]">
                  <th scope="col" className="py-3 pl-5 pr-4 text-left text-[10px] font-bold tracking-[0.15em] uppercase text-[#9CA3AF]">User</th>
                  <th scope="col" className="py-3 px-4 text-left text-[10px] font-bold tracking-[0.15em] uppercase text-[#9CA3AF] hidden md:table-cell">Role</th>
                  <th scope="col" className="py-3 px-4 text-left text-[10px] font-bold tracking-[0.15em] uppercase text-[#9CA3AF] hidden lg:table-cell">Dept. Scope</th>
                  <th scope="col" className="py-3 pl-4 pr-5 text-right text-[10px] font-bold tracking-[0.15em] uppercase text-[#9CA3AF]">Actions</th>
                </tr>
              </thead>
                ...
                {loading ? (
                  ...
                  <tr><td colSpan={4}>
                    <div className="flex flex-col items-center justify-center py-16 text-center">
                      ...
                      <p className="text-sm font-semibold text-[#374151]">No users found</p>
                          ...
                          <div className="flex items-center gap-3">
                            ...
                            <div className="min-w-0">
                              <div className="flex items-center gap-1.5">
                                ...
                                {isMe && (
                                  <span className="shrink-0 rounded-full bg-[#F3F4F6] px-1.5 py-0.5 text-[10px] font-bold text-[#6B7280] ring-1 ring-[#E5E7EB]">You</span>
                                )}
                              </div>
                              <p className="text-[11px] text-[#9CA3AF] truncate">{u.email}</p>
                              {u.uwiId && <p className="text-[11px] text-[#9CA3AF]">ID: {u.uwiId}</p>}
                            </div>
                          </div>
                        </td>
                          ...
                          {u.role === "admin" ? (
                            <div className="flex flex-wrap gap-1.5">
                              {(u.scopedDepartmentIds ?? []).length === 0 ? (
                                <span className="text-xs text-[#9CA3AF] italic">No scope assigned</span>
                              ) : (
                                u.scopedDepartmentIds.map((deptId) => {
                                  const d = departments.find((x) => x.id === deptId);
                                  return (
                                    <span key={deptId} className="rounded-full bg-[#F3F4F6] px-2 py-0.5 text-[11px] text-[#374151] ring-1 ring-[#E5E7EB]">
                                      {d?.name ?? `#${deptId}`}
                                    </span>
                                  );
                                })
                        ...
                        <td className="py-3.5 pl-4 pr-5">
                          {isMe ? (
                            <div className="flex justify-end">
                              <span className="text-xs text-[#9CA3AF] italic">Current account</span>
                            </div>
                          ) : isBusy ? (
                            <div className="flex justify-end">
                            ...
                            <div className="flex items-center justify-end gap-1.5 flex-wrap">
                              ...
                              {u.role === "admin" && (
                                <>
                                  <button
                                    onClick={() => { setScopeUser(u); setScopeModalOpen(true); }}
                                    className="rounded-lg border border-[#E5E7EB] bg-white px-2.5 py-1.5 text-[11px] font-semibold text-[#374151] hover:border-[#003595] hover:bg-[#EAF6FF] hover:text-[#003595] transition-all whitespace-nowrap">
                                    Assign Dept.
                                  </button>
                                  ...
                                  <button onClick={() => updateRole(u.id, "student")}
      ...
      <ScopeModal
        ...
        onClose={() => { setScopeModalOpen(false); setScopeUser(null); }}
